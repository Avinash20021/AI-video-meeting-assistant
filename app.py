...
# Force Python to look inside the Core folder first to prevent root-level import clashes
...
with st.sidebar:
    ...
    cookie_path = st.text_input("Optional cookies file path", placeholder="C:/path/to/cookies.txt")
    st.caption(
        "Use public YouTube videos only. Restricted/private videos need a cookies file via "
        "YTDLP_COOKIEFILE in .env or enter a local cookies file path above."
    )
...
if run_btn:
    if not source.strip():
        ...
        try:
            ...
            chunks = process_input(source, cookie_path)
            ...
            st.session_state.result = {
                ...
                "retriever": retriever,  # store retriever, NOT rag_chain
            }
...
if st.session_state.result:
    ...
    if send_btn and user_input.strip():
        with st.spinner("Thinking…"):
            # Rebuild rag_chain fresh each time (chains aren't serializable)
            rag_chain = build_rag_chain(r["retriever"])
